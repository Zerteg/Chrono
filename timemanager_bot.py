...
from sqlite import db_start, add_task, del_task
...
async def start(message: types.Message):
    ...
    await add_task(user_id=message.from_user.id)
...
async def trash(message: types.Message):
    ...
    await del_task(state, user_id=message.from_user.id)

@dp.message_handler(state=ProfileStatesGroup.description,text = ['Напоминания'])
